...
func TestNewJWTValidator(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:                "GIVEN jwt validator options with HS algorithms allowed and no hmacSecret set EXPECT error code jwt validator missing signer",
			jwtValidatorOptions: JWTValidatorOptions{},
			expectedErrorCode:   coreerrors.ErrCodeJWTValidatorMissingSigner,
		},
		{
			name: "GIVEN jwt validator options with audience required true and no allowed audiences provided and allowAnyAudience false EXPECT error code jwt validator audience required but none provided",
			jwtValidatorOptions: JWTValidatorOptions{
				HMACOptions: HMACSigningOptions{
					Secret: "test",
				},
...
func TestValidateHeader(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name: "GIVEN a header with an algorithm that is not supported by the signer EXPECT error code jwt algorithm not allowed",
			jwtValidatorOptions: JWTValidatorOptions{
				HMACOptions: HMACSigningOptions{
					Secret: "test secret",
				},
		...
		// keyid tests
		{
			name: "GIVEN a header with a key id while key id is not required EXPECT no errors to occur",
			jwtValidatorOptions: JWTValidatorOptions{
				HMACOptions: HMACSigningOptions{
					Secret: "test secret",
				},
