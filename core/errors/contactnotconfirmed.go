...
func NewContactNotConfirmedError(contactId string, principal string, principalType string, fields map[string]interface{}, includeStack bool) errors.RichError {
	...
	err := errors.NewRichError(ErrCodeContactNotConfirmed, msg).WithMetaData(fields).AddMetaData("contactId", contactId).AddMetaData("principal", principal).AddMetaData("principalType", principalType)
	if includeStack {
		err = err.WithStack(1)
	}
	return err
