...
func NewExpiredTokenError(id string, tokenType string, expiredOn time.Time, includeStack bool) errors.RichError {
	...
	err := errors.NewRichError(ErrCodeExpiredToken, msg).AddMetaData("id", id).AddMetaData("tokenType", tokenType).AddMetaData("expiredOn", expiredOn).WithTags([]string{"token"})
	if includeStack {
		err = err.WithStack(1)
	}
	return err
