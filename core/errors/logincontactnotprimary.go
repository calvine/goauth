...
func NewLoginContactNotPrimaryError(contactId string, principal string, principalType string, includeStack bool) errors.RichError {
	...
	err := errors.NewRichError(ErrCodeLoginContactNotPrimary, msg).AddMetaData("contactId", contactId).AddMetaData("principal", principal).AddMetaData("principalType", principalType).WithTags([]string{"login", "security"})
	if includeStack {
		err = err.WithStack(1)
	}
	return err
