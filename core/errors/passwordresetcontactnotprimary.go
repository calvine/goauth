...
func NewPasswordResetContactNotPrimaryError(contactId string, principal string, principalType string, includeStack bool) errors.RichError {
	...
	err := errors.NewRichError(ErrCodePasswordResetContactNotPrimary, msg).AddMetaData("contactId", contactId).AddMetaData("principal", principal).AddMetaData("principalType", principalType).WithTags([]string{"login"})
	if includeStack {
		err = err.WithStack(1)
	}
	return err
